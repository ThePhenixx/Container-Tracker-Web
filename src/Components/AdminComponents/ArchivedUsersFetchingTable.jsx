...
export default function ArchivedUsersFetchingTable(props){
    ...
    useEffect( () => {
        if(parameter !== ""){
            fetch("http://localhost:8080/api/v1/users-controller/archived-users/" + parameter, {
                method: 'GET',
                headers: {
            ...
            fetch("http://localhost:8080/api/v1/users-controller/all-archived-users", {
                method: 'GET',
                headers: {
                    "Authorization": "Bearer " + localStorage.getItem("authenticationToken"),
                    "Content-Type": "application/json"
    ...
    const Redirect = (rgNumber) => {
        navigate("/archived-user-page/" + rgNumber);
    }
